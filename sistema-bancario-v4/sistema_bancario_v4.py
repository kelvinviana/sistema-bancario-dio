...
class ContasIterador:
    ...
    def __next__(self):
        try:
            ...
            Limite:\t\tR$ {conta._limite if hasattr(conta, '_limite') else '0.00'}
            Saques disponíveis:\t{conta._limite_saques - len([t for t in conta.historico.transacoes if t['tipo'] == 'Saque']) if hasattr(conta, '_limite_saques') else 'N/A'}
        """
        except IndexError:
            raise StopIteration
        finally:
...
def exibir_extrato(clientes):
    ...
    saldo_total = sum(conta.saldo for conta in cliente.contas)
    print(f"\nSaldo Total: R$ {saldo_total:.2f}")

    # Extrato para cada conta
    for conta in cliente.contas:
        print(f"\nConta: Agência {conta.agencia} Número {conta.numero}")
        ...
        for transacao in conta.historico.gerar_relatorio():
            tem_transacao = True
            print(f"{transacao['tipo']} em {transacao['data']}: R$ {transacao['valor']:.2f}")
